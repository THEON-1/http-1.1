...
int main(int argc, char *argv[]) {
  ...
  pthread_t *free_thread = malloc(sizeof(*free_thread));

  check_malloc(free_thread);
  ...
  while (1) {
    ...
    if ((connection =
             accept(sockfd, &connection_data, &connection_data_size)) == -1) {
      perror("accept");
      exit(1);
    }
    ...
    args->data =
        (connection_data_t){.connection = connection,
